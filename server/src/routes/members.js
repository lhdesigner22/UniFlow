...
async function isPipeAdmin(pipeId, userId) {
...
router.post('/', async (req, res) => {
  ...
  const exists = await db.prepare('SELECT id FROM pipe_members WHERE pipe_id = ? AND user_id = ?').get(req.params.pipeId, user.id)
  if (exists) return res.status(409).json({ error: 'Já é membro' })
  const id = uuid()
  await db.prepare('INSERT INTO pipe_members (id, pipe_id, user_id, role) VALUES (?, ?, ?, ?)').run(id, req.params.pipeId, user.id, role)
  res.json({ id, user, role })
})

router.put('/:memberId', async (req, res) => {
  const { role } = req.body
  await db.prepare('UPDATE pipe_members SET role = ? WHERE id = ?').run(role, req.params.memberId)
  res.json({ ok: true })
})

router.delete('/:memberId', async (req, res) => {
  await db.prepare('DELETE FROM pipe_members WHERE id = ?').run(req.params.memberId)
  res.json({ ok: true })
})
...
router.get('/routing', async (req, res) => {
  if (!await isPipeAdmin(req.params.pipeId, req.user.id))
...
router.post('/routing', async (req, res) => {
  ...
  const id = uuid()
  try {
    await db.prepare('INSERT INTO pipe_routing_rules (id, pipe_id, from_user_id, to_user_id) VALUES (?, ?, ?, ?)')
...
router.delete('/routing/:ruleId', async (req, res) => {
  if (!await isPipeAdmin(req.params.pipeId, req.user.id))
    return res.status(403).json({ error: 'Apenas admins podem remover regras de encaminhamento.' })

  await db.prepare('DELETE FROM pipe_routing_rules WHERE id = ? AND pipe_id = ?')
...
router.get('/routing-groups', async (req, res) => {
  if (!await isPipeAdmin(req.params.pipeId, req.user.id))
...
router.post('/routing-groups', async (req, res) => {
  if (!await isPipeAdmin(req.params.pipeId, req.user.id))
  ...
  const id = uuid()
  try {
    await db.prepare('INSERT INTO pipe_routing_groups (id, pipe_id, from_user_id, group_type, group_value) VALUES (?, ?, ?, ?, ?)')
...
router.delete('/routing-groups/:groupId', async (req, res) => {
  if (!await isPipeAdmin(req.params.pipeId, req.user.id))
    return res.status(403).json({ error: 'Apenas admins podem remover grupos de encaminhamento.' })

  await db.prepare('DELETE FROM pipe_routing_groups WHERE id = ? AND pipe_id = ?')
