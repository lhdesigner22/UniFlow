...
router.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM automations WHERE pipe_id = ? ORDER BY created_at DESC').all(req.params.pipeId))
})

router.post('/', (req, res) => {
  const { name, trigger_type, trigger_config, action_type, action_config } = req.body
  const id = uuid()
  db.prepare('INSERT INTO automations (id, pipe_id, name, trigger_type, trigger_config, action_type, action_config) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
router.put('/:id', (req, res) => {
  ...
  db.prepare('UPDATE automations SET name=?, trigger_type=?, trigger_config=?, action_type=?, action_config=?, active=? WHERE id=?')
  ...
  res.json(db.prepare('SELECT * FROM automations WHERE id = ?').get(a.id))
})
