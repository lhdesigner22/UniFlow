...
router.post('/', async (req, res) => {
  ...
  const id = uuid()
  try {
    await db.prepare('INSERT INTO pipe_custom_groups (id, pipe_id, name, color) VALUES (?, ?, ?, ?)')
...
router.put('/:groupId', async (req, res) => {
  ...
  const g = await db.prepare('SELECT * FROM pipe_custom_groups WHERE id = ? AND pipe_id = ?')
    .get(req.params.groupId, req.params.pipeId)
  if (!g) return res.status(404).json({ error: 'Grupo não encontrado.' })

  const { name, color } = req.body
  try {
    await db.prepare('UPDATE pipe_custom_groups SET name = ?, color = ? WHERE id = ?')
      .run(name ?? g.name, color ?? g.color, g.id)
  } catch {
    return res.status(409).json({ error: 'Já existe um grupo com esse nome.' })
  }

  res.json(await db.prepare('SELECT * FROM pipe_custom_groups WHERE id = ?').get(g.id))
})
...
router.delete('/:groupId', async (req, res) => {
  ...
  await db.prepare('DELETE FROM pipe_custom_group_members WHERE group_id = ?').run(req.params.groupId)
  await db.prepare('DELETE FROM pipe_routing_groups WHERE group_type = ? AND group_value = ?')
    .run('custom', req.params.groupId)
  await db.prepare('DELETE FROM pipe_custom_groups WHERE id = ? AND pipe_id = ?')
...
router.post('/:groupId/members', async (req, res) => {
  ...
  const pipeMember = await db.prepare('SELECT id FROM pipe_members WHERE pipe_id = ? AND user_id = ?')
    .get(req.params.pipeId, user_id)
  if (!pipeMember) return res.status(400).json({ error: 'Usuário não é membro deste pipe.' })

  const id = uuid()
  try {
    await db.prepare('INSERT INTO pipe_custom_group_members (id, group_id, user_id) VALUES (?, ?, ?)')
      .run(id, req.params.groupId, user_id)
  } catch {
    return res.status(409).json({ error: 'Usuário já pertence a este grupo.' })
  }

  const user = await db.prepare('SELECT id, name, email, avatar FROM users WHERE id = ?').get(user_id)
...
router.delete('/:groupId/members/:userId', async (req, res) => {
  ...
  await db.prepare('DELETE FROM pipe_custom_group_members WHERE group_id = ? AND user_id = ?')
